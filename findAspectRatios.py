...
def findAspectRatios(video):
	...
	if(deliveryType == 'static_origin'):
		response = GetCMS().GetRenditionList(videoID=videoID)
	elif(deliveryType == 'dynamic_origin'):
		response = GetCMS().GetDynamicRenditions(videoID=videoID)
	else:
	...
	if(response.status_code in GetCMS().success_responses):
		...
		if(sourceH and sourceW):
			x,y = aspect_ratio(sourceW, sourceH)
